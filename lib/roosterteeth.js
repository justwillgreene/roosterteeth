...
module.exports = function (parameters) {
  ...
  var lists = ['watchlist'];
  var notifies = ['notifications'];
  var svodBes = ['bonus_features', 'channels', 'episodes', 'featured_items', 'livestreams', 'marketing_banners', 'schedule', 'seasons', 'shows', 'store_carousel'];
  var wtcs = ['played_positions'];
  ...
  notifies.forEach(function (notify) {
    self[notify] = require(__dirname + '/sources/notify/' + notify + '.js')(self);
  });
  ...
  wtcs.forEach(function (wtc) {
    self[wtc] = require(__dirname + '/sources/wtc/' + wtc + '.js')(self);
  });
};
